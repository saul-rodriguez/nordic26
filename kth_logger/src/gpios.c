...
int configure_KTH_logger_gpio(){
	...
	if (!gpio_is_ready_dt(&out1)) {
        LOG_ERR("Output 1 not ready");    
        return -1;
    }

    err = gpio_pin_configure_dt(&out1, GPIO_OUTPUT_INACTIVE);
	if (err < 0) {
        LOG_ERR("out1 failed to configure");    
        return err;
    }

    //configuration out2
    if (!gpio_is_ready_dt(&out2)) {
	...
	if (err < 0) {
        LOG_ERR("out2 failed to configure");            
        return err;
    }
...
{
    int err;
    err = gpio_pin_set_dt(&out1, 1);

    if (err){
        LOG_WRN("Cannot set Output 1 (%d)", err);            
    }
    return err;
}
...
{
    int err;
    err = gpio_pin_set_dt(&out2, 1);

    if (err){
        LOG_WRN("Cannot set Output 2 (%d)", err);            
    }
    return err;
